...
export function buildMainlinePrompt(cards) {
  const system = [
    ...
    "要求：把它们串成一条线索，说明共同指向什么变化或趋势；不要罗列条目，不要复述标题，要给出判断；不超过 120 字；直接输出这段文字，不要 JSON；不要以「今日主线」之类的字样开头。",
  ].join("\n");
...
export function buildTakeawaysPrompt(cards) {
  const system = [
    "你是 AI 产品经理的情报编辑。",
    "下面是今天日报的全部条目，请提炼「今日三条收获」。",
    '要求：每条一句话，不超过 40 字，是判断而不是事实复述；站在产品经理视角；只输出 JSON：{"takeaways": ["...", "...", "..."]}。',
  ].join("\n");
...
export async function summarizeTakeaways(cards, { config, postJsonImpl = postJson, retries = 1 } = {}) {
  if (!config?.apiKey) {
    throw new Error("缺少 LLM_API_KEY，无法生成收获");
  }

  const timeoutMs = config.timeoutMs ?? 60000;
  const { system, user } = buildTakeawaysPrompt(cards);
  let lastError;
