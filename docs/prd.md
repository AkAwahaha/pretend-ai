...
3. **收藏**：本地存储，无需账号
4. **历史日报**：读取归档，按日期回看
5. **我的灵感**：记录独立笔记，也可关联某条新闻；支持编辑、删除和本地持久化
...
- 我的灵感：首页快捷记录 + 独立笔记页面；支持独立笔记、可选关联新闻、编辑删除和本地持久化
- 掌握状态：首页卡片和详情页都能手动标记「已掌握 / 未掌握」；首页显示已处理进度；未掌握内容进入独立复习队列
- 20 个信息源抓取、去重、已发布过滤
- 按主题分类与筛选
- 每条包含：来源、主题、标题、概述、亮点思路、产品视角、原文链接、阅读时长
- 手机适配网页，支持收藏与历史回看
- 支持导出 Markdown（单条精读导出、整日日报导出）
- Obsidian 接入代码保留，但当前版本从界面隐藏
- 静态站点，每天自动更新
...
| v1.9 | 2026-09-16 | 删除今日收获，避免其与今日主线重复 |
| v2.0 | 2026-09-16 | 新增「我的灵感」：首页快捷记录、独立笔记页、可选关联新闻、编辑删除与本地持久化 |
| v2.1 | 2026-09-16 | 掌握状态形成闭环：已处理进度、未掌握独立复习队列、重复点击保持状态，全部处理后当日学习结束并有明确待复习数量 |
| v2.2 | 2026-09-16 | 将详情页「记下灵感」移动到三问卡片和原文链接之后，作为内容最后一节 |
| v2.3 | 2026-09-16 | 恢复 Obsidian 界面入口；文章导出自动包含关联灵感；“我的灵感”支持批量导入 Obsidian |
